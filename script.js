$(document).ready(function () {
  ...
  window.indexStr = 0;
  window.uuid = uuidv4();
  window.str = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyzÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚÝàáâãèéêìíòóôõùúýĂăĐđĨĩŨũƠơƯưẠạẢảẤấẦầẨẩẪẫẬậẮắẰằẲẳẴẵẶặẸẹẺẻẼẽẾếỀềỂểỄễỆệỈỉỊịỌọỎỏỐốỒồỔổỖỗỘộỚớỜờỞởỠỡỢợỤụỦủỨứỪừỬửỮữỰựỲỳỴỵỶỷỸỹ!”%&’()+,-./0123456789:;=?@";
  $('.txt')[0].innerText = str[window.indexStr];
...
function downloadImg() {
  ...
  $.ajax({
    ...
    data: { 
       imgBase64: img,
       fileName: window.str[window.indexStr - 1] === '/' ? 'gach_cheo' : window.str[window.indexStr - 1],
       userId: window.uuid
    }
  }).done(function(o) {
    console.log('saved'); 
